...
call ordStatusProducts('Shipped');
call ordStatusProducts('Cancelled');
call ordStatusProducts('On Hold');
select cube1(5);
select distinct productLine, max(MSRP) over(partition by productLine) as MAX_MSRP from products;
select highestMSRP('Motorcycles');
select highestCreditCustomer();
